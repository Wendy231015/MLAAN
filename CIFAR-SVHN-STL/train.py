...
import networks.resnetInfoPro
import networks.resnetDGL
import networks.resnetDGLbin
import networks.resnetInfoProbin
import networks.resnetDGL_EMAPlus
import networks.resnetDGL_DFLocalPlus
import networks.resnetMLAAN
import networks.resnetInfoPro_EMAPlus
import networks.resnetInfoPro_DFLPlus
...
record_path = './' \
              + ('LayerMoCo*_' if args.balanced_memory else 'LayerMoCo_') \
              + str(args.dataset) \
...
def main():
    ...
    kwargs = {'num_workers': 10, 'pin_memory': False}
    ...
    if args.model == 'resnet':
        model = eval('networks.resnetMLAAN.resnet' + str(args.layers))\
    ...
    for epoch in range(start_epoch, training_configurations[args.model]['epochs']):
        ...
        torch.save(model, record_path + '/model.pth')
