...
parser.add_argument('--net', default='resnet101', type=str,
                    help='networks: [resnet34|resnet101|resnet152]')
...
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                    help='momentum')
parser.add_argument('--momentum2', default=0.995, type=float, metavar='M',
                    help='momentum')
...
# torch.save(net, mymodel.pth)#保存整个model的状态
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = eval('networks.' + args.arch + '.' + args.net)(momentum = args.momentum2,local_module_num = args.local_module_num)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(tqdm(train_loader)):
        ...
        output, loss = model(x=images,
                             target=target,
                             criterion=criterion,)
        # loss.backward()  因为resnet34里面已经backword过了
        optimizer.step()
